...
local function rawPostRequest(url, rawdata, auth)
	local response = {}
	local headers = {}
...
local function oauth_nonce()
   return mime.b64(hmac.digest("sha1",tostring(math.random()) .. "random" .. tostring(os.time()), "keyyyy", true))
end
...
local function oauth_sign(url, method, args, consumer_secret)
   ...
   -- print("token secret", token_secret)
   --
   ...
   if method == "GET" then
      ...
      return string.format('OAuth oauth_consumer_key="%s", oauth_nonce="%s", oauth_signature="%s", oauth_signature_method="%s", oauth_timestamp="%s", oauth_token="%s", oauth_version="%s"', 
      	 encode_parameter(args.oauth_consumer_key),
      	 encode_parameter(args.oauth_nonce),
      	 encode_parameter(hmac_b64),
      	 encode_parameter(args.oauth_signature_method),
      	 encode_parameter(args.oauth_timestamp),
      	 encode_parameter(args.oauth_token),
      	 encode_parameter(args.oauth_version)
      	 )
   end
end
...
local function getRequestToken(options)
    ...
    local result, code, headers = rawPostRequest(options.request_token_url, post_data)
...
local function getAccessByTokenAndSecret(token, token_secret, options)
    ...
    local function local_call(url, method, params)
        ...
        if (method == "POST") then
        	--post_data.oauth_timestamp = "1383745342"
        	--post_data.oauth_nonce = "d379bf81061dd125cbfcd57e4efb9370"
            local auth = oauth_sign(
                url,
                "POST",
                post_data,
                --token_secret
		    ...
		    for key, value in pairs(params) do
		        table.insert(key_value_pairs, encode_parameter(key) .. "=" .. encode_parameter(value))
		    end
   			local query_string = table.concat(key_value_pairs, "&")
            return rawPostRequest(url, query_string, auth)
...
local function getAccessToken(token, verifier, token_secret, options)
    ...
    local post_data = oauth_sign(options.access_token_url,
                          ...
                          post_data,
                          options.consumer_secret)
    local result, code, headers = rawPostRequest(options.access_token_url, post_data)
...
local function getAccessTokenByPin(pin, options)
    ...
    assert(options.consumer_secret,"consumer_secret option must be set")
    assert(options.access_token_url,"access_token_url option must be set")
    assert(options.request_token,"request_token option must be set")
    ...
    local post_data = {
       ...
       oauth_version      = '1.0',
       oauth_nonce        = oauth_nonce(),
       oauth_token        = options.request_token,
...
function newConsumer(options)
    ...
    function local_getAccessTokenByPin(pin, request_token)
        local access_opts = {
            consumer_key = consumer_key,
            consumer_secret = consumer_secret,
            access_token_url = access_token_url,
